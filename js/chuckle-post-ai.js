...
function ChucklePostAI(AI_option) {
  ...
  async function recommendList() {
    ...
    if(sessionStorage.getItem('recommendList')){
      ...
      try {
        response = await fetch(`https://summary.tianli0.top/recommends?url=${encodeURIComponent('https://tianli-blog.club/cdn-auth/')}&author=all`, options)
